...
class Authentication extends Singleton {
	...
	protected function getUserModules() {
		$sql = "SELECT m.id as id, m.name as name FROM ac_module as m 
			INNER JOIN ac_user_module as um ON um.module_id = m.id 
			INNER JOIN ac_user as u ON um.user_id = u.id
			WHERE u.public_id = :user_id";
		$params[":user_id"] = $this->getPublicId();
		return db()->FetchRows($sql, $params, $this);
	}
	...
	public function fetchUserById($id) {
		$sql = "select {$this->tableName()}.*, `ac_module`.`public_id` as `mod_pubid`, `ac_module`.`name` as 'module_name' from {$this->tableName()} inner join `ac_module` on `ac_module`.`id`={$this->tableName()}.`default_module` where {$this->tableName()}.`id`=:id ";
		$params = array(
			":id" => $id,
		);
	...
	public function getRecord($id = false) {
		if ($id) {
			return $this->fetchUserById();
		}
